"""core_static JSON-seed bridge.

Mirrors the per-module pattern (see admin_user_seed_loader, session_seed_loader).
Reads `permission_titles_seed.json` and wraps each entry into the core_seeds
shape `RbacRoleService.seed_rbac_from_module()` understands.
"""
...
def load_core_static_permission_titles() -> list[dict[str, Any]]:
