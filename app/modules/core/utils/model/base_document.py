...
from app.modules.core.models.field_translation_keys import DEFAULT_LANGUAGE, SUPPORTED_LANGUAGE_CODES, FALLBACK_LANGUAGE
...
class BaseDocument(Document, BaseModel, TimestampMixin, ABC):
    ...
    Translation Registration:
        Each model can define its own FIELD_TRANSLATION_KEYS as a class attribute:

        FIELD_TRANSLATION_KEYS = FieldTranslationHelper.create_keys(
            fr={"field_name": "Label FR"},
            en={"field_name": "Label EN"},
            ln={"field_name": "Label LN"},
        )

        These are auto-registered at class definition time and used by
        TranslationService.get_static_fields_translation().

        Enum translations belong in {module_name}_field_translation_keys.py and are
        merged into core TRANSLATIONS at import time (see configuration_field_translation_keys.py).
    """

    # Per-model translation declarations (override in subclasses)
    FIELD_TRANSLATION_KEYS: ClassVar[Dict[str, Dict[str, str]]] = {}

    # Global registry (auto-populated via __init_subclass__)
    _field_translation_registry: ClassVar[Dict[str, Dict[str, str]]] = {}

    def __init_subclass__(cls, **kwargs):
        super().__init_subclass__(**kwargs)
        # Register field translations
        field_keys = cls.__dict__.get("FIELD_TRANSLATION_KEYS", {})
        if field_keys:
            # Auto-fill missing languages (in case create_keys was not used)
    ...
    def handle_translation_status(self, status_value, enum_cls, accept_language: str = DEFAULT_LANGUAGE) -> str:
        ...
        try:
            ...
            # Resolve translation mapping from core TRANSLATIONS
        ...
        def _walk(value: Any) -> Any:
            ...
            for key, child in list(result.items()):
                ...
                if isinstance(child, (dict, list)):
                    result[key] = _walk(child)
                elif encrypted_fields:
                    # Fallback: decrypt any remaining encrypted strings that
                    # weren't covered by explicit metadata (helps if field
                    # metas drift). Only attempt when the model actually
                    # declares encryptable fields — otherwise the string
                    # was likely encrypted by a different service/key.
    ...
    async def _format_unwind_document(
        ...
        async def _format_nested(model_name: str, value: Any) -> Any:
            ...
            async def _format_single_item(item: Dict[str, Any]) -> Any:
                # Prefer full validation so optional fields (foreign-key IDs like
                # sys_application_id, sys_menu_id, etc.) survive into the formatted
                # output. The required-only path used to drop them, which broke
                # downstream tree builders that key on those FKs.
                nested_instance = None
                try:
                    nested_instance = nested_model_class.model_validate(item)
                except Exception as e_full:
                    DebugService.app_debug_print(
                        f"Full validate failed for nested model {model_name}: {e_full}; "
                        f"falling back to required-only instantiation",
                        False,
                    )
                    try:
                        ...
                        DebugService.app_debug_print(
                            f"Error instantiating nested model {model_name}: {e_req}",
                            False,
                        )
                        # Last-resort fallback: return item with ID normalized
    ...
    async def save_with_hooks(self, **kwargs) -> "BaseDocument":
        ...
        # Ensure a MongoDB view exists for this collection
    ...
    async def _ensure_view_exists(self):
        """Ensure that a MongoDB view exists for this document's collection.

        Uses the same DB connection as the DAO (base.py mongodb singleton)
        so all code paths share one client. Awaited so the view is guaranteed
        to exist before the save response reaches the caller.
        """
        ...
        try:
            collection_name = self.get_motor_collection().name
            ...
            # Create the view using the shared mongodb connection
            pipeline = [
            ...
            DebugService.app_debug_print(f"✅ Vue créée instantanément pour {collection_name} (via BaseDocument)", True)

        except Exception as e:
            from app.modules.core.services.debug.debug_service import DebugService
            DebugService.app_debug_print(f"⚠️ BaseDocument._ensure_view_exists failed: {str(e)}", True)
