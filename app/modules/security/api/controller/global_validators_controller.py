...
from app.modules.security.enums.security_enum import ESudoActionAccessTypeFlag, ESudoActionAccessTargetedTypeFlag
...
class GlobalValidatorsController(DebugService, ResponseService, ConverterService, AuthenticatedService):
    ...
    async def fetch_global_validators(
        ...
        """
        Fetch all CfgSudoActionAccess records where
        sudo_action_access_type == GLOBAL_ACCESS.
        These are users or groups that have global validator privileges.
        """
        try:
            ...
            # Force filter to global access type
            query_params["filter__sudo_action_access_type"] = ESudoActionAccessTypeFlag.GLOBAL_ACCESS.value
            ...
            formatted_data = []
            for item in data:
                self.app_debug_print(f"item sudo group : {item}",False)
                instance = CfgSudoActionAccessModel(**item)
            ...
            format_error = format_exception(f"error fetch_global_validators ",e)
            self.app_debug_print(f"Error in fetch_global_validators: > 1 {format_error} {exception_line_info(e)}",True)
    ...
    async def fetch_available_users(
        ...
        try:
            ...
            existing_user_validators = await self.generic_service.fetch_data_from_collection(
                ...
                accept_language=self.accept_language,
                query={
                    "filter__sudo_action_access_type": ESudoActionAccessTypeFlag.GLOBAL_ACCESS.value,
