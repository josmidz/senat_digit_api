...
from app.modules.security.api.controller.cross_validators_controller import CrossValidatorsController
...
@router.get("/fetch/cross-validators")
async def fetch_cross_validators(
    ...
    """Fetch all org sudo action configurations formatted as RBAC title tree with linked cross-validators."""
    accept_language = request.headers.get("accept-language", DEFAULT_LANGUAGE).split(",")[0].strip()
    controller = CrossValidatorsController(accept_language)
    return await controller.fetch_cross_validators(
        request=request,
...
@router.get("/fetch/available-orgs")
async def fetch_available_orgs(
    ...
    accept_language = request.headers.get("accept-language", DEFAULT_LANGUAGE).split(",")[0].strip()
    controller = CrossValidatorsController(accept_language)
    return await controller.fetch_available_orgs(
        request=request,
        output_data_type=output_data_type,
        all_data=all_data,
        page=page,
