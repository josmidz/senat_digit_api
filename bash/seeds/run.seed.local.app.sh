#!/bin/bash
# bash/seeds/run.seed.local.app.sh
# Exit on error
set -e
...
SEED_OUT_LOG="$LOGS_DIR/app_local_seed_out.log"
SEED_ERR_LOG="$LOGS_DIR/app_local_seed_err.log"
...
# NOTE (2026-04-29): the §3.4 brand sweep dropped the TRANSCO `configuration`
# module — `app.modules.configuration.seeds.data.configuration_apps_seed`
# no longer resolves and was silently aborting the run under `set -e`.
# Senat-Digit's app/menu catalog is owned by the senat-digit module loaders
# wired into `init_data()` via `senat_digit_modules_rbac.py` (per F2).
