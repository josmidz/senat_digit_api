...
#
# NOTE (2026-04-29): the §3.4 brand sweep dropped the TRANSCO-domain
# `configuration`, `urban_transportation`, and `trans_agent_flutter_app`
# modules (none apply to Senat-Digit). The python invocations that
# referenced them were silently aborting the run under `set -e` because
# their module paths no longer resolve. Removed.
#
# `app.modules.core.seeds.seed.init_data()` is the canonical seed entry
# point — it walks every refdata + RBAC + senat-digit module loader
# (post-F2) in one shot. Everything else is L2 cache pre-warming.
SEED_OUT_LOG="$LOGS_DIR/local_seed_out.log"
SEED_ERR_LOG="$LOGS_DIR/local_seed_err.log"
...
USER_APP_STORE_SEED_OUT_LOG="$LOGS_DIR/local_user_app_store_seed_out.log"
USER_APP_STORE_SEED_ERR_LOG="$LOGS_DIR/local_user_app_store_seed_err.log"
...
USER_APP_STORE_DYN_OUT_LOG="$LOGS_DIR/local_user_app_store_dynamic_seed_out.log"
USER_APP_STORE_DYN_ERR_LOG="$LOGS_DIR/local_user_app_store_dynamic_seed_err.log"
