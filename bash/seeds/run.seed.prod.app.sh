#!/bin/bash
# bash/seeds/run.seed.prod.app.sh
# Exit on error
set -e
...
if [ -f .env.production ]; then
  ...
  if grep -q -E "^\s*[^#=]+=" .env.production; then
    echo "Loading local environment variables from .env.production..."
...
SEED_OUT_LOG="$LOGS_DIR/app_prod_seed_out.log"
SEED_ERR_LOG="$LOGS_DIR/app_prod_seed_err.log"
