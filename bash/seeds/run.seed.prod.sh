...
# Validate and load environment variables from the .env.production file
if [ -f .env.production ]; then
  echo "Validating .env.production file..."
  # Check if the file contains only valid KEY=VALUE pairs
  if grep -q -E "^\s*[^#=]+=" .env.production; then
    ...
    echo "Error: .env.production file contains invalid lines. Ensure all lines are valid KEY=VALUE pairs."
    exit 1
  fi
else
...
SEED_OUT_LOG="$LOGS_DIR/prod_seed_out.log"
SEED_ERR_LOG="$LOGS_DIR/prod_seed_err.log"
...
USER_APP_STORE_SEED_OUT_LOG="$LOGS_DIR/prod_user_app_store_seed_out.log"
USER_APP_STORE_SEED_ERR_LOG="$LOGS_DIR/prod_user_app_store_seed_err.log"
...
USER_APP_STORE_DYN_OUT_LOG="$LOGS_DIR/prod_user_app_store_dynamic_seed_out.log"
USER_APP_STORE_DYN_ERR_LOG="$LOGS_DIR/prod_user_app_store_dynamic_seed_err.log"
