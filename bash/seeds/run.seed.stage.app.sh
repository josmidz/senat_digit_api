#!/bin/bash
# bash/seeds/run.seed.stage.app.sh
# Exit on error
set -e
...
# Validate and load environment variables from the .env.stage file
if [ -f .env.stage ]; then
  echo "Validating .env.stage file..."
  # Check if the file contains only valid KEY=VALUE pairs
  if grep -q -E "^\s*[^#=]+=" .env.stage; then
    ...
    echo "Error: .env.stage file contains invalid lines. Ensure all lines are valid KEY=VALUE pairs."
    exit 1
  fi
else
...
SEED_OUT_LOG="$LOGS_DIR/app_stage_seed_out.log"
SEED_ERR_LOG="$LOGS_DIR/app_stage_seed_err.log"
